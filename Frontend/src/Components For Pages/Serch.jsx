                ...
                <div className="search-result">
                    {serch.map((items, index) => (
                        <div key={index} className="search-result-item">
                            <svg className='search-result-icon' width="20" height="20" viewBox="0 0 24 24" fill="#5f6368">
                                <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" />
                            </svg>
                            <NavLink className='to-connect' to={'/connectionReq'} state={{ value: items }}>
                                <div className='to-connectios' onClick={() => navigate('/context', { state: { value: items } })}>
                                    <span className='search-user-email'>{items.email}</span>
                                </div>
                            </NavLink>
                        </div>
                    ))
